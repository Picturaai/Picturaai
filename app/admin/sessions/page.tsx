...
export default function AdminSessionsPage() {
  ...
  const loadSessions = async () => {
    ...
    try {
      ...
      if (!res.ok) return setMessage(data.error || 'Failed to load sessions')
    ...
    <div className="min-h-screen bg-background text-foreground">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Session/IP/device/location visibility + credit controls.</p>
              </div>
            </div>
      ...
      <main className="mx-auto grid w-full max-w-6xl gap-4 px-4 py-4 sm:grid-cols-12 sm:px-6">
        <section className="rounded-2xl border border-border/60 bg-card p-4 sm:col-span-7">
          <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
            <div className="relative">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search session, IP, country" className="h-10 w-full rounded-lg border border-border/60 bg-background pl-9 pr-3 text-sm outline-none focus:border-primary" />
            </div>
            <button onClick={loadSessions} disabled={loading} className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-primary px-4 text-sm font-medium text-primary-foreground disabled:opacity-60">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}Load</button>
          </div>

          <div className="mt-4 overflow-hidden rounded-xl border border-border/60">
            <div className="max-h-[420px] overflow-auto">
              <table className="w-full min-w-[620px] text-left text-xs">
                <thead className="sticky top-0 bg-secondary/60"><tr><th className="px-3 py-2 font-semibold">Session</th><th className="px-3 py-2 font-semibold">Image left</th><th className="px-3 py-2 font-semibold">Video left</th><th className="px-3 py-2 font-semibold">Device / Location / IP</th></tr></thead>
                <tbody>{rows.map((row) => {const imageLeft = Math.max(0, limits.image - row.credits_used); const videoLeft = Math.max(0, limits.video - row.video_used); const active = row.session_id === selectedSession; return (<tr key={row.session_id} className={`cursor-pointer border-t border-border/40 ${active ? 'bg-primary/10' : 'hover:bg-secondary/40'}`} onClick={() => setSelectedSession(row.session_id)}><td className="px-3 py-2 font-mono text-[11px]">{row.session_id}</td><td className="px-3 py-2">{imageLeft}</td><td className="px-3 py-2">{videoLeft}</td><td className="px-3 py-2">{row.last_device || 'unknown'} • {row.last_city || '-'}, {row.last_region || '-'} {row.last_country || '-'} • {row.last_ip || '-'}</td></tr>)})}</tbody>
              </table>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-border/60 bg-card p-4 sm:col-span-5">
          ...
          <p className="mt-2 text-xs text-muted-foreground">Current daily limits for your role: image {limits.image} / video {limits.video}</p>
          ...
          <div className="mt-3 rounded-lg border border-border/60 bg-background p-2 text-xs">
            ...
            <p className="truncate"><span className="text-muted-foreground">User agent:</span> {selected?.last_user_agent || '-'}</p>
          </div>
          ...
          <div className="mt-4 grid gap-2">
            ...
            <div className="grid gap-2 sm:grid-cols-2"><input value={imageDelta} onChange={(e) => setImageDelta(e.target.value)} placeholder="Add image credits" className="h-10 rounded-lg border border-border/60 bg-background px-3 text-sm" /><button onClick={() => updateSession({ imageDelta: Number(imageDelta || 0) })} disabled={loading || !selectedSession} className="h-10 rounded-lg border border-border/60 bg-background px-3 text-sm font-medium hover:bg-secondary/60 disabled:opacity-60">Apply</button></div>
            <div className="grid gap-2 sm:grid-cols-2"><input value={videoDelta} onChange={(e) => setVideoDelta(e.target.value)} placeholder="Add video credits" className="h-10 rounded-lg border border-border/60 bg-background px-3 text-sm" /><button onClick={() => updateSession({ videoDelta: Number(videoDelta || 0) })} disabled={loading || !selectedSession} className="h-10 rounded-lg border border-border/60 bg-background px-3 text-sm font-medium hover:bg-secondary/60 disabled:opacity-60">Apply</button></div>
            <div className="grid gap-2 sm:grid-cols-2"><input value={imageRemaining} onChange={(e) => setImageRemaining(e.target.value)} placeholder="Set image remaining" className="h-10 rounded-lg border border-border/60 bg-background px-3 text-sm" /><button onClick={() => updateSession({ imageRemaining: Number(imageRemaining || 0) })} disabled={loading || !selectedSession} className="h-10 rounded-lg border border-border/60 bg-background px-3 text-sm font-medium hover:bg-secondary/60 disabled:opacity-60">Set</button></div>
            <div className="grid gap-2 sm:grid-cols-2"><input value={videoRemaining} onChange={(e) => setVideoRemaining(e.target.value)} placeholder="Set video remaining" className="h-10 rounded-lg border border-border/60 bg-background px-3 text-sm" /><button onClick={() => updateSession({ videoRemaining: Number(videoRemaining || 0) })} disabled={loading || !selectedSession} className="h-10 rounded-lg border border-border/60 bg-background px-3 text-sm font-medium hover:bg-secondary/60 disabled:opacity-60">Set</button></div>
          </div>
