...
import { ArrowRight, Layers, Globe, FlaskConical, Cpu, Shield, BarChart3, BookOpen, Microscope, GitBranch, Check, X, MapPin, CircleDollarSign, Image as ImageIcon, Clock, Clapperboard, Music, ChevronDown, Wand2, Lightbulb, Maximize2, Aperture, Type, Bell, Mic2, SlidersHorizontal, Download, Star, CircleDot, Users } from 'lucide-react'
...
import { formatCompactCount, GITHUB_ISSUES_URL, GITHUB_REPO_URL } from '@/lib/github'
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          >
            <div className="relative rounded-2xl border border-border/40 bg-card shadow-2xl shadow-primary/[0.07] overflow-hidden">
              ...
              {/* Output area — real, playable cinematic preview */}
              <VideoPreviewPlayer
                src="/videos/picturagen-preview.mp4"
                poster="/videos/picturagen-preview-poster.jpg"
                seed="42891"
              />
            ...
            <p className="mt-4 text-center text-xs text-muted-foreground">
              Playable demo clip · PicturaGen v2.5 Text to Video
            </p>
          </motion.div>
      ...
      <section className="relative overflow-hidden border-t border-border/40 py-20 md:py-28">
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          >
            <div className="relative rounded-2xl border border-border/40 bg-card shadow-2xl shadow-primary/[0.07] overflow-hidden">
              ...
              <div className="relative bg-secondary/20 p-4 sm:p-6">
                <AudioPreviewPlayer
                  ...
                  subtitle="Synthwave · Instrumental demo · 112 BPM"
                  cover="/images/showcase-hero-2.jpg"
                  lyrics={
                    <>
                      <span className="text-muted-foreground/60">[Verse 1]</span> Headlights tracing every line we drew,{' '}
                      <span className="rounded bg-primary/10 px-1 text-primary">midnight runs the city through&hellip;</span>
                    </>
                  }
                />
              </div>
            ...
            {/* Caption */}
            <p className="mt-4 text-center text-xs text-muted-foreground">
              Playable demo track · PicturaSound v1, a separate Pictura service for music
            </p>
          </motion.div>
...
/** Open source callout: repo stats straight from GitHub plus ways to contribute. */
function OpenSourceSection() {
  const { stats, loading } = useRepoStats()

  const repoStats = [
    { Icon: Star, label: 'Stars', value: stats.stars },
    { Icon: GitBranch, label: 'Forks', value: stats.forks },
    { Icon: Users, label: 'Watchers', value: stats.watchers },
    { Icon: CircleDot, label: 'Open issues', value: stats.openIssues },
  ]

  return (
    <section className="relative overflow-hidden border-t border-border/40 bg-secondary/20 py-20 md:py-28">
      <div className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[320px] bg-[radial-gradient(ellipse_60%_60%_at_50%_-10%,var(--primary)/0.10,transparent_70%)]" />
      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        >
          ...
          <p className="mt-4 text-base leading-relaxed text-muted-foreground text-pretty">
            The Studio, the API, and PicturaCAPTCHA all live in one public repository. Read the code, run it
            yourself, open an issue, or send a pull request &mdash; and star the repo if Pictura is useful to you.
          </p>
        </motion.div>
        ...
        <motion.div
          ...
          custom={1}
          variants={fadeUp}
          className="mx-auto mt-10 max-w-3xl rounded-2xl border border-border/50 bg-card p-6 md:p-8"
        >
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl bg-foreground text-background">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Next.js &middot; TypeScript &middot; Tailwind &mdash; the whole product, in the open.
                </p>
              </div>
            </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 border-t border-border/40 pt-6 sm:grid-cols-4">
            {repoStats.map(({ Icon, label, value }) => (
              <div key={label} className="rounded-xl border border-border/40 bg-background px-4 py-3">
                <div className="flex items-center gap-1.5 text-muted-foreground">
                  <Icon className="h-3.5 w-3.5" />
                  <span className="text-[11px] font-medium">{label}</span>
                </div>
                {loading && value === null ? (
                  <span className="mt-1.5 block h-5 w-10 animate-pulse rounded bg-border" aria-hidden="true" />
                ) : (
                  <p className="mt-1 text-xl font-bold tabular-nums text-foreground">{formatCompactCount(value)}</p>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-muted-foreground">
            <a href={GITHUB_ISSUES_URL} target="_blank" rel="noopener noreferrer" className="transition-colors hover:text-primary">
              Report an issue
            </a>
            <a href={`${GITHUB_REPO_URL}/fork`} target="_blank" rel="noopener noreferrer" className="transition-colors hover:text-primary">
              Fork the repo
            </a>
            <a href={`${GITHUB_REPO_URL}/pulls`} target="_blank" rel="noopener noreferrer" className="transition-colors hover:text-primary">
              Open pull requests
            </a>
          </div>
        </motion.div>
